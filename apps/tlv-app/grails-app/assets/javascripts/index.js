...
function convertGeospatialCoordinateFormat(inputString, callbackFunction) {
	...
	if (inputString.match(dmsPattern)) {
		...
		var latitude = coordinateConversion.dmsToDd(
			degreesLat,
			minutesLat,
			secondsLat,
			directionLat
		...
		var longitude = coordinateConversion.dmsToDd(
			degreesLon,
			minutesLon,
			secondsLon,
			directionLon
		...
		if (callbackFunction) {
			callbackFunction([longitude, latitude]);
		} else {
		...
		inputString.match(mgrsPattern) &&
		inputString.trim().length < 16
	) {
		var location = coordinateConversion.mgrsToDd(
			RegExp.$1,
			RegExp.$2,
			RegExp.$3,
			RegExp.$4,
		...
		inputString.match(bePattern) &&
		inputString.trim().length == 10 &&
		tlv.beLookup &&
		tlv.beLookup.url &&
		callbackFunction
	) {
		displayLoadingDialog(
			"We're checking our maps for that location... BRB!"
		);
		beSearch(inputString)
			...
			.done(function (data) {
				if (data.features.length > 0) {
					...
					displayErrorDialog("We couldn't find that BE. :(");
				}
			});
	} else {
...
function convertRadiusToBbox(x, y, radius) {
	...
	var deltaLongitude =
		(radius / 1852 / 60) * Math.cos((Math.abs(y) * Math.PI) / 180);

	return {
		maxLat: y + deltaLatitude,
		maxLon: x + deltaLongitude,
		minLat: y - deltaLatitude,
		minLon: x - deltaLongitude,
	};
}
...
function disableMenuButtons() {
	...
	for (var i = 2; i < menuButtons.length - 2; i++) {
		$(menuButtons[i]).hide();
	}
...
function displayDialog(dialog) {
	...
	var maxBodyHeight =
		($(window).height() - paddingHeight - headerHeight - footerHeight) *
		0.9;
...
function displayNavbar() {
	...
	if (tlv.hideNavbar) {
		clearTimeout(tlv.hideNavbar);
	}
	tlv.hideNavbar = setTimeout(function () {
		...
		if (!$(".modal-backdrop").is(":visible")) {
			switch (keyCode) {
				...
				case 65:
					if (event.shiftKey) {
						var degrees =
							(tlv.map.getView().getRotation() * 180) / Math.PI -
							1;
						tlv.map
							.getView()
							.setRotation((degrees * Math.PI) / 180);
					} else {
						changeFrame("rewind");
					}
					break;
...
function groundToImagePoints(coordinates, layer, callback) {
	return $.ajax({
		...
		var pixels = data.data.map(function (point) {
...
function imagePointsToGround(pixels, layer, options, callback) {
	...
	return $.ajax({
		...
		var coordinates = data.data.map(function (point) {
