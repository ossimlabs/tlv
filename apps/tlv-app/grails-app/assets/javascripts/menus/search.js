function adjustLastDaysDate() {
	...
	Object.getPrototypeOf($("#endDateTimePicker")).size = function () {
		return this.length;
	...
	Object.getPrototypeOf($("#searchStartDateTimePicker")).size = function () {
		return this.length;
	}; // Workaround for https://github.com/Eonasdan/bootstrap-datetimepicker/issues/1714
...
function beginSearch() {
				...
				$.each(searchParams.libraries, function (index, library) {
					...
					if (tlv.libraries[library].wfsUrlProxy) {
						params.url = encodeURIComponent(
							tlv.libraries[library].wfsUrlProxy +
								"?" +
								$.param(params)
						);
							...
							if (data.features.length) {
								...
								$.each(
									...
									function (index, feature) {
										...
										images.push({
											acquisitionDate: processAcquisitionDate(
								...
								$.each(
									...
									function (index, library) {
										searchesComplete.push(
											library.searchComplete
										);
									}
								);
	...
	// check that location param is what we expect [lon,lat] or an empty string and filter
	if (
		(!isLatitude(searchParams.location[1]) ||
			!isLongitude(searchParams.location[0])) &&
		locationString != ""
	) {
		searchParams.inputError = true;
	} else {
		searchParams.inputError = false;
	}

	// added input check in error check
	if (searchParams.inputError || searchParams.error) {
		...
		displayLoadingDialog(
			"We are searching the libraries for imagery... fingers crossed!"
		);
		...
		$.each(searchParams.libraries, function (index, library) {
			...
			if (tlv.filter) {
				...
				var startDate =
					...
					":" +
					searchParams.startMinute +
					":" +
					searchParams.startSecond +
					".000'";
				var endDate =
					"'" +
					...
					searchParams.endDay +
					"T" +
					searchParams.endHour +
					":" +
					searchParams.endMinute +
					":" +
					searchParams.endSecond +
					".999'";
				filter +=
					"((acquisition_date >= " +
					startDate +
			...
			if (tlv.libraries[library].wfsUrlProxy) {
				data.url = encodeURIComponent(
					tlv.libraries[library].wfsUrlProxy + "?" + $.param(data)
				);
				url = tlv.contextPath + "/home/proxy";
			}
...
function coverageSearch() {
	...
	if (!params.location) {
		...
		convertGeospatialCoordinateFormat(
	...
	$.ajax({
		...
		if (data.length) {
			...
			$("#coverageTable").html(
				"We couldn't find any additional coverage. :("
			);
		}
	});
}
...
function coverageStatusCheck() {
	$("#coverageTable")
		...
		.each(function (index, div) {
			...
			getWfs(params, url).done(function (data) {
...
function getDistinctSensors() {
	var updateSensorSelect = function () {
		...
		buttonGroup.html("");
		$.each(
			[].concat.apply([], sensors).unique().sort(),
			function (index, sensor) {
		...
		if (tlv.sensors || tlv.preferences.tlvPreference.sensor) {
			selectedSensors = (
				tlv.sensors || tlv.preferences.tlvPreference.sensor
			)
				.split(",")
				.map(function (sensor) {
					return sensor.trim();
...
function getEndDate() {
	var date = $("#searchEndDateTimePicker")
		.data("DateTimePicker")
		.date()
		.toDate();
...
function getSearchParams() {
	...
	searchObject.fsgNot = $("#searchFsgNotCheckbox").hasClass("active")
		? true
		: false;
...
function getSelectedLibraries() {
	...
	if (Object.keys(tlv.libraries).length == 1) {
		...
		$.each(tlv.libraries, function (key, value) {
...
function getStartDate() {
	var date = $("#searchStartDateTimePicker")
		.data("DateTimePicker")
		.date()
		.toDate();
...
function initializeEndDateTimePicker() {
	...
	var endDateTimePicker = $("#searchEndDateTimePicker");
	Object.getPrototypeOf($("#searchEndDateTimePicker")).size = function () {
		return this.length;
	}; // Workaround for https://github.com/Eonasdan/bootstrap-datetimepicker/issues/1714
	endDateTimePicker.datetimepicker({
		date: endDate,
...
function initializeLibraryCheckboxes() {
	...
	if (tlv.searchLibraries) {
		$.each(tlv.searchLibraries.split(","), function (index, library) {
			var checkbox = $(
				"#searchLibrary" + library.capitalize() + "Checkbox"
			);
		...
		var checkbox = $(
			"#searchLibrary" +
				Object.keys(tlv.libraries)[0].capitalize() +
				"Checkbox"
		);
		checkbox.trigger("click");
	}
...
function initializeMaxCloudCoverInput() {
	...
	maxCloudCover =
		tlv.preferences.tlvPreference.maxCloudCover || maxCloudCover;
...
function initializeMaxResultsSelect() {
	...
	$("#searchMaxResultsSelect option[value = '" + maxResults + "']").prop(
		"selected",
		true
	);
}
...
function initializeStartDateTimePicker() {
	...
	var endDate = $("#searchEndDateTimePicker")
		.data("DateTimePicker")
		.date()
		.toDate();
	...
	if (tlv.preferences.tlvPreference.startDate) {
		startDate = new Date(
			Date.parse(tlv.preferences.tlvPreference.startDate)
		);
	}

	if (tlv.startYear) {
		startDate.setFullYear(tlv.startYear);
	}
	if (tlv.startDay) {
		startDate.setDate(tlv.startDay);
	}
	if (tlv.startMonth) {
		startDate.setMonth(tlv.startMonth - 1);
	}
	...
	Object.getPrototypeOf($("#startDateTimePicker")).size = function () {
		return this.length;
	}; // Workaround for https://github.com/Eonasdan/bootstrap-datetimepicker/issues/1714
	...
	$.ajax({
		...
		.done(function (data) {
			...
			var orderImageStatusCheck = function (feature, progressBar) {
				var timeCheck =
					(parseFloat(progressBar.style.width) / 100) * 60;
				if (timeCheck == 0) {
					...
					getWfs(params, url).done(function (data) {
						if (data.features.length > 0) {
							...
							progressBar.onclick = function () {
								window.open(
									tlv.contextPath + "?" + $.param(params)
								);
							};
						} else {
							progressBar.innerHTML =
								feature.imageId + ": Checking in 60 sec...";
							setTimeout(function () {
...
function processResults() {
	...
	var searchesComplete = [];
	$.each(tlv.libraries, function (key, library) {
		searchesComplete.push(library.searchComplete);
	});

	if (searchesComplete.indexOf(false) > -1) {
		return;
	} else {
		hideLoadingDialog();
		...
		if (searchResults.length > 0) {
			tlv.layers = searchResults.sort(function (a, b) {
				if (a.acquisitionDate > b.acquisitionDate) {
					return -1;
				} else if (a.acquisitionDate < b.acquisitionDate) {
					return 1;
				}

				// if the acquisiton dates are the same, sort by entry id
				else if (a.metadata.entry_id < b.metadata.entry_id) {
					return -1;
				} else if (a.metadata.entry_id > b.metadata.entry_id) {
					return 1;
				}
			...
			var maxResults = $("#searchMaxResultsSelect").val();
			if (tlv.layers.length > maxResults) {
				tlv.layers.splice(maxResults);
			}
			...
			if (!tlv.location) {
				var geometries = tlv.layers.map(function (layer) {
					return new ol.format.WKT().readGeometry(
						layer.metadata.footprint
					);
				});

				var extent = new ol.geom.GeometryCollection(
					geometries
				).getExtent();
			...
			displayErrorDialog(
				"Sorry, we couldn't find anything that matched your search criteria. Maybe ease up on those search constraints a bit?"
			);
		}
	}
}
